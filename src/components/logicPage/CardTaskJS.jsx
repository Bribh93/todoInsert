...
const CardTaskJs = () => {
  ...
  async function deleteCard(id) {
    await supabase.from("jscode").delete().eq("id", id);
          ...
          {task.map((tasks) => (
            ...
            >
              <div className="flex items-center justify-between ">
                ...
                <span className="bg-yellow-500/10 text-yellow-500 font-semibold md:py-2 md:px-4 py-1 px-2 text-xs rounded uppercase">
                  {tasks.languages?.trim() || "Js"}
                </span>
              </div>
              <div className="flex flex-col gap-2 mb-4">
                <Link
                  href="/"
                  className=" font-bold hover:underline mt-2 text-yellow-500 line-clamp-1 md:line-clamp-2 "
                >
                  ✔️{tasks.title}
                </Link>
              ...
              <div className="flex justify-between items-center mt-4">
                <button
                  ...
                  className="border hover:bg-yellow-600/30 text-yellow-600 font-semibold md:py-2 md:px-4 py-1 px-2 text-xs rounded uppercase transition-colors duration-300 "
                >
                  Update
                </button>
                <button
            ...
            <div className="fixed inset-0 bg-black/70 bg-opacity-50 flex items-center justify-center z-50 transition-transform duration-500 ">
              <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-[90%] max-w-md max-h-[80vh] overflow-y-auto text-white">
                ...
                <button
                  ...
                  className="absolute top-2 right-2  bg-yellow-500 text-white px-4 py-2 rounded-full hover:bg-green-600 transition-colors duration-300"
                >
                  X
                </button>
              </div>
