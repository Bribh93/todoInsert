...
const CardTasksCss = () => {
  ...
  async function deleteCard(id) {
    await supabase.from("csscode").delete().eq("id", id);
      ...
      <div className="flex justify-center mt-10 ">
        <div className="md:min-w-[800px] max-h-[450px] md:max-h-[600px] overflow-y-auto scroll-smooth snap-y snap-mandatory">
          {task.map((tasks) => (
            <div
              key={tasks.id}
              className="bg-background-secondary max-w-2xl mx-auto py-4 px-6 rounded-lg shadow-xl shadow-blue-600/10 md:hover:shadow-blue-600/50 mb-16 relative md:hover:scale-95 transform transition duration-300"
            >
              <div className="flex items-center justify-between ">
                <time className="text-sm text-muted-foreground">
                  {formatDate(tasks.created_at)}
                </time>
                <span className="bg-blue-600/10 text-blue-600 font-semibold md:py-2 md:px-4 py-1 px-2 text-xs rounded uppercase">
                  {tasks.languages?.trim() || "Css"}
                </span>
              </div>
              <div className="flex flex-col gap-2 mb-4">
                <Link
                  href="/"
                  className=" text-blue-600 font-bold hover:underline mt-2 md:line-clamp-2 line-clamp-1"
                >
                  ✔️{tasks.title}
                </Link>
            ...
            <div className="fixed inset-0 bg-black/70 bg-opacity-50 flex items-center justify-center z-50 transition-transform duration-500">
              <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-[90%] max-w-md max-h-[80vh] overflow-y-auto text-white">
                ...
                <button
                  ...
                  className="absolute top-2 right-2 md:top-32 md:right-4/12 bg-blue-600 text-white px-4 py-2 rounded-full hover:bg-blue-600 transition-colors duration-300"
                >
                  X
                </button>
              </div>
