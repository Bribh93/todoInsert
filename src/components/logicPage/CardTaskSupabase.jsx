...
const CardTaskSupabase = () => {
  ...
  async function deleteCard(id) {
    await supabase.from("supabasecode").delete().eq("id", id);
          ...
          {task.map((tasks) => (
            ...
            >
              <div className="flex items-center justify-between ">
                ...
                <span className="bg-green-600/10 text-green-600 font-semibold md:py-2 md:px-4 py-1 px-2 text-xs rounded uppercase">
                  {tasks.languages?.trim() || "Supabase"}
                </span>
              </div>
              <div className="flex flex-col gap-2 mb-4">
                <Link
                  href="/"
                  className=" font-bold hover:underline mt-2 text-green-600 line-clamp-1"
                >
                  ✔️{tasks.title}
                </Link>
              ...
              <div className="flex justify-between items-center mt-4">
                <button
                  ...
                  className="border hover:bg-green-600/30 text-green-600 font-semibold md:py-2 md:px-4 py-1 px-2 text-xs rounded uppercase transition-colors duration-300 "
                >
                  Update
                </button>
                <button
            ...
            <div className="fixed inset-0 bg-black/70 bg-opacity-50 flex items-center justify-center z-50 transition-transform duration-500 ">
              <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-[90%] max-w-md max-h-[80vh] overflow-y-auto text-white">
                ...
                <button
                  ...
                  className="absolute top-[10%] right-2  md:right-[20%] xl:right-[35%] 2xl:right-[44%]  bg-green-500 text-white px-4 py-2 rounded-full hover:bg-green-600 transition-colors duration-300"
                >
                  X
                </button>
              </div>
...
export default CardTaskSupabase;
