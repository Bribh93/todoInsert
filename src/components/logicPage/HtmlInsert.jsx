  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("htmlcode")
        .insert([
          { title: taskName, description: description, languages: showLabel, user_id: user.id },
        ])
        .select();
  ...
  return (
    <div className="grid justify-center items-center max-w-2xl mx-auto">
      ...
      >
        ...
        <button
          type="submit"
          className="w-[50%] bg-transparent border-2 border-orange-500/50 text-orange-500 py-2 px-5 rounded-full hover:bg-orange-600 hover:text-white transition-colors"
        >
          Add
        </button>
      </form>
